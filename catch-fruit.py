...
class ScoreBoard():
    ...
    def gameOver(self):
        self.saveScore()
        tkinter.messagebox.showinfo("message", "No more lives, u just got out!")
        if tkinter.messagebox.askyesno("message", "Play Again?"):
            self.reset()
        else:
            exit()
...
class TheGame(ItemsFallingFromSky,ScoreBoard):
    ...
    def __init__(self,parent):
        ...
        self.parent.title("bot fuit hunt ")
        ...
        # player character
        self.playerPhoto = tkinter.PhotoImage(file = "images/{}" .format( "3w1vko.gif" ) )
